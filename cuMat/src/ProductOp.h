 ...
 */
template <typename _LeftScalar, typename _RightScalar, ProductArgOp _OpLeft, ProductArgOp _OpRight,
          ProductArgOp _OpOutput>
...
struct traits<ProductOp<_Left, _Right, _OpLeft, _OpRight, _OpOutput> >
{
  ...
  using Scalar =
      typename internal::ProductElementFunctor<LeftScalar, RightScalar, _OpLeft, _OpRight, _OpOutput>::Scalar;
  enum
  {
 ...
 */
template <typename _Left, typename _Right, internal::ProductArgOp _OpLeft, internal::ProductArgOp _OpRight,
          internal::ProductArgOp _OpOutput>
...
public:
  ...
  {
    CUMAT_STATIC_ASSERT(
        (std::is_same<typename internal::NumTraits<typename internal::traits<_Left>::Scalar>::ElementalType,
    ...
    {
      CUMAT_ASSERT_ARGUMENT((TransposedLeft ? left_.rows() : left_.cols()) ==
                            (TransposedRight ? right_.cols() : right_.rows()));
    }
    CUMAT_STATIC_ASSERT(
        CUMAT_IMPLIES((ColumnsLeft >= 1 && RowsRight >= 1),
    ...
    CUMAT_STATIC_ASSERT(!(BatchesLeft > 1 && BatchesRight > 1) || (BatchesLeft == BatchesRight),
...
public:
  ...
  */
  ...
  typedef ProductOp<_Left, _Right, _OpLeft, _OpRight,
                    internal::ProductArgOp((int)(_OpOutput) ^ (int)(internal::ProductArgOp::TRANSPOSED))>
      transposed_mult_t;
  ...
  CUMAT_STRONG_INLINE __device__ Scalar coeff(Index row, Index col, Index batch, Index index) const
  {
    ...
    LeftScalar left = left_.coeff(TransposedLeft ? 0 : r, TransposedLeft ? r : 0,
                                  internal::traits<Type>::BroadcastBatchesLeft ? 0 : batch, -1);
    ...
    RightScalar right = right_.coeff(TransposedRight ? c : 0, TransposedRight ? 0 : c,
                                     internal::traits<Type>::BroadcastBatchesRight ? 0 : batch, -1);
    right = ConjugateRight ? internal::NumOps<RightScalar>::conj(right) : right;
    // output
    Scalar output =
        internal::ProductElementFunctor<LeftScalar, RightScalar, _OpLeft, _OpRight, _OpOutput>::mult(left, right);
...
namespace internal
{
template <typename _Dst, typename _DstTag, ProductArgOp _DstOp, typename _SrcLeft, typename _SrcLeftTag,
          ProductArgOp _SrcLeftOp, typename _SrcRight, typename _SrcRightTag, ProductArgOp _SrcRightOp,
          AssignmentMode _AssignmentMode>
struct ProductAssignment;
//{
//    using Op = ProductOp<_SrcLeft, _SrcRight, _SrcLeftOp, _SrcRightOp, _DstOp>;
//    static void assign(_Dst& dst, const Op& op) { CUMAT_STATIC_ASSERT(false, "Product not implemented for these
//    arguments"); }
//};
...
struct Assignment<_Dst, _Src, _AssignmentMode, _DstTag, ProductSrcTag>
{
  static void assign(_Dst& dst, const _Src& src)
  {
    ...
    ProductAssignment<typename _Dst::Type, _DstTag, internal::ProductArgOp(SrcActual::OutputOp),
                      typename SrcActual::LeftType, typename traits<typename SrcActual::LeftType>::SrcTag,
                      internal::ProductArgOp(SrcActual::LeftOp), typename SrcActual::RightType,
                      typename traits<typename SrcActual::RightType>::SrcTag,
                      internal::ProductArgOp(SrcActual::RightOp), _AssignmentMode>::assign(dst.derived(),
                                                                                           src.derived());
  }
};
...
template <typename _Dst, ProductArgOp _DstOp, typename _SrcLeft, ProductArgOp _SrcLeftOp, typename _SrcRight,
          ProductArgOp _SrcRightOp, AssignmentMode _AssignmentMode>
struct ProductAssignment<_Dst, DenseDstTag, _DstOp, _SrcLeft, CwiseSrcTag, _SrcLeftOp, _SrcRight, CwiseSrcTag,
                         _SrcRightOp, _AssignmentMode>
{
  using Op = ProductOp<_SrcLeft, _SrcRight, _SrcLeftOp, _SrcRightOp, _DstOp>;
  ...
  static void evalImpl(_Dst& mat, float betaIn, const left_wrapped_t& left, const right_wrapped_t& right, const Op& op,
                       std::integral_constant<bool, true> /*direct-write*/)
  {
    ...
    Scalar beta __attribute__((aligned(16))) = betaIn;
#endif
    ...
    if (Op::Batches > 1 || op.batches() > 1)
    {
      ...
      internal::CublasApi::current().cublasGemmBatched(
          transA, transB, m, n, k, internal::CublasApi::cast(&alpha), internal::CublasApi::cast(A), lda, strideA,
          internal::CublasApi::cast(B), ldb, strideB, internal::CublasApi::cast(&beta), internal::CublasApi::cast(C),
          ldc, strideC, internal::narrow_cast<int>(op.batches()));
    }
    else
    {
      // single non-batched evaluation
      internal::CublasApi::current().cublasGemm(transA, transB, m, n, k, internal::CublasApi::cast(&alpha),
                                                internal::CublasApi::cast(A), lda, internal::CublasApi::cast(B), ldb,
                       ...
                       std::integral_constant<bool, false> /*non-direct-write*/)
  {
    ...
    ProductAssignment<DstTmp, DenseDstTag, _DstOp, _SrcLeft, CwiseSrcTag, _SrcLeftOp, _SrcRight, CwiseSrcTag,
                      _SrcRightOp, AssignmentMode::ASSIGN>::evalImpl(tmp, betaIn, left, right, op,
  ...
  static void assign(_Dst& dst, const Op& op)
  {
    ...
    using DirectWriteTag =
        std::integral_constant<bool, (int(traits<_Dst>::AccessFlags) & int(AccessFlags::WriteDirect)) != 0>;
 ...
 * This is a specialization if both input matrices are transposed. This avoids the explicit evaluation of \c
 * .transpose() . \tparam _Left the left type \tparam _Right the right type \param left the left matrix \param right the
 * right matrix \return the result of the matrix-matrix multiplication
 */
template <typename _Left, typename _Right>
ProductOp<_Left, _Right, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::TRANSPOSED,
          internal::ProductArgOp::NONE>
operator*(const TransposeOp<_Left, false>& left, const TransposeOp<_Right, false>& right)
{
  return ProductOp<_Left, _Right, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::TRANSPOSED,
                   internal::ProductArgOp::NONE>(left.getUnderlyingMatrix(), right.getUnderlyingMatrix());
}

/**
 * \brief Performs the matrix-matrix multiplication <tt>C = A * B'<tt>.
 * This is a specialization if the right input matri is transposed. This avoids the explicit evaluation of \c
 * .transpose() . \tparam _Left the left type \tparam _Right the right type \param left the left matrix \param right the
 * right matrix \return the result of the matrix-matrix multiplication
 */
template <typename _Left, typename _Right>
...
{
  return ProductOp<_Left, _Right, internal::ProductArgOp::NONE, internal::ProductArgOp::TRANSPOSED,
                   internal::ProductArgOp::NONE>(left, right.getUnderlyingMatrix());
}

/**
 * \brief Performs the matrix-matrix multiplication <tt>C = A' * B<tt>.
 * This is a specialization if the left input matri is transposed. This avoids the explicit evaluation of \c
 * .transpose() . \tparam _Left the left type \tparam _Right the right type \param left the left matrix \param right the
 * right matrix \return the result of the matrix-matrix multiplication
 */
template <typename _Left, typename _Right>
...
{
  return ProductOp<_Left, _Right, internal::ProductArgOp::TRANSPOSED, internal::ProductArgOp::NONE,
                   internal::ProductArgOp::NONE>(left.getUnderlyingMatrix(), right);
}
...
{
  return ProductOp<_Left, _Right, internal::ProductArgOp::NONE, internal::ProductArgOp::NONE,
                   internal::ProductArgOp::NONE>(left, right);
}
