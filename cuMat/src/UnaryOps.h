...
DEFINE_GENERAL_FUNCTOR(cwiseInverseCheck,
                       (x == _Scalar(0) ? _Scalar(1) : CUMAT_NAMESPACE internal::NumTraits<_Scalar>::RealType(1) / x));
...
#else
DEFINE_FUNCTOR_FLOAT(cwiseRsqrt, (x));  // Fallback to prevent the error that rsqrt and rcbrt are not found if not
                                        // compiled with NVCC
...
DEFINE_FUNCTOR(cwiseFloor, int, x);
DEFINE_FUNCTOR(cwiseFloor, long, x);
;
DEFINE_FUNCTOR(cwiseCeil, int, x);
DEFINE_FUNCTOR(cwiseCeil, long, x);
;
DEFINE_FUNCTOR(cwiseRound, int, x);
DEFINE_FUNCTOR(cwiseRound, long, x);
;
...
public:
  explicit CastingOp(const MatrixBase<_Child>& child) : child_(child.derived())
  {
  }
...
struct traits<AsDiagonalOp<_Child>>
{
  ...
  {
    Flags = internal::traits<_Child>::Flags,
    Size = internal::traits<_Child>::RowsAtCompileTime == 1 ? internal::traits<_Child>::ColsAtCompileTime :
...
public:
  explicit AsDiagonalOp(const MatrixBase<_Child>& child)
    : child_(child.derived()), size_(child.rows() == 1 ? child.cols() : child.rows())
  {
    CUMAT_STATIC_ASSERT(internal::traits<_Child>::RowsAtCompileTime == 1 ||
                            internal::traits<_Child>::ColsAtCompileTime == 1,
...
struct traits<ExtractDiagonalOp<_Child>>
{
  ...
  using Scalar = typename ExtractDiagonalFunctor<MatrixType>::VectorType;  // using Scalar = typename
                                                                           // internal::traits<_Child>::Scalar;
  enum
  {
    Flags = internal::traits<_Child>::Flags,
    RowsAtCompileTime = (internal::traits<_Child>::RowsAtCompileTime == Dynamic ||
                         internal::traits<_Child>::RowsAtCompileTime == Dynamic) ?
                            Dynamic :
...
public:
  explicit ExtractDiagonalOp(const MatrixBase<_Child>& child)
    : child_(child.derived()), size_(child.rows() < child.cols() ? child.rows() : child.cols())
  {
  }
...
public:
  explicit ExtractComplexPartOp(const MatrixBase<_Child>& matrix) : matrix_(matrix.derived())
  {
  }
...
public:
  explicit ExtractComplexPartOp(MatrixBase<_Child>& matrix) : matrix_(matrix.derived())
  {
    CUMAT_STATIC_ASSERT(internal::traits<_Child>::AccessFlags & WriteCwise,
                        "Lvalue version of ExtractComplexPartOp must be called on a matrix expression with the "
                        "WriteCwise-Flag");
  }
  ...
  CUMAT_STRONG_INLINE Type& operator=(const MatrixBase<Derived>& expr)
  {
    ...
    internal::Assignment<Type, Derived, AssignmentMode::ASSIGN, internal::DenseDstTag,
                         typename internal::traits<Derived>::SrcTag>::assign(*this, expr.derived());
    return *this;
  }
};
...
struct traits<SwapAxisOp<_Child, _Row, _Col, _Batch>>
{
  ...
  {
    Flags = internal::traits<_Child>::Flags,
    RowsAtCompileTime = _Row == Axis::Row ? internal::traits<_Child>::RowsAtCompileTime :
                                            _Row == Axis::Column ?
                                            internal::traits<_Child>::ColsAtCompileTime :
                                            _Row == Axis::Batch ? internal::traits<_Child>::BatchesAtCompileTime : 1,
    ColsAtCompileTime = _Col == Axis::Row ? internal::traits<_Child>::RowsAtCompileTime :
                                            _Col == Axis::Column ?
                                            internal::traits<_Child>::ColsAtCompileTime :
                                            _Col == Axis::Batch ? internal::traits<_Child>::BatchesAtCompileTime : 1,
    BatchesAtCompileTime = _Batch == Axis::Row ?
                               internal::traits<_Child>::RowsAtCompileTime :
                               _Batch == Axis::Column ?
                               internal::traits<_Child>::ColsAtCompileTime :
                               _Batch == Axis::Batch ? internal::traits<_Child>::BatchesAtCompileTime : 1,
    AccessFlags = ReadCwise
  };
  typedef CwiseSrcTag SrcTag;
  typedef DeletedDstTag DstTag;
...
public:
  explicit SwapAxisOp(const MatrixBase<_Child>& child) : child_(child.derived())
  {
  }
  ...
  __host__ __device__ CUMAT_STRONG_INLINE Index rows() const
  {
    return _Row == Axis::Row ? child_.rows() :
                               _Row == Axis::Column ? child_.cols() : _Row == Axis::Batch ? child_.batches() : 1;
  }
  __host__ __device__ CUMAT_STRONG_INLINE Index cols() const
  {
    return _Col == Axis::Row ? child_.rows() :
                               _Col == Axis::Column ? child_.cols() : _Col == Axis::Batch ? child_.batches() : 1;
  }
  __host__ __device__ CUMAT_STRONG_INLINE Index batches() const
  {
    return _Batch == Axis::Row ? child_.rows() :
                                 _Batch == Axis::Column ? child_.cols() : _Batch == Axis::Batch ? child_.batches() : 1;
  }

  __device__ CUMAT_STRONG_INLINE Scalar coeff(Index row, Index col, Index batch, Index index) const
  {
    return child_.derived().coeff(
        _Row == Axis::Row ? row : _Col == Axis::Row ? col : _Batch == Axis::Row ? batch : 0,
        _Row == Axis::Column ? row : _Col == Axis::Column ? col : _Batch == Axis::Column ? batch : 0,
        _Row == Axis::Batch ? row : _Col == Axis::Batch ? col : _Batch == Axis::Batch ? batch : 0, -1);
  }
};
...
#define UNARY_OP(Name, Op)                                                                                             \
  ...
  CUMAT_NAMESPACE UnaryOp<_Derived, CUMAT_NAMESPACE functor::UnaryMathFunctor_##Op<                                    \
                                        typename CUMAT_NAMESPACE internal::traits<_Derived>::Scalar>>                  \
  Name(const CUMAT_NAMESPACE MatrixBase<_Derived>& mat)                                                                \
  {                                                                                                                    \
    CUMAT_ERROR_IF_NO_NVCC(Op)                                                                                         \
    return CUMAT_NAMESPACE UnaryOp<_Derived, CUMAT_NAMESPACE functor::UnaryMathFunctor_##Op<                           \
                                                 typename CUMAT_NAMESPACE internal::traits<_Derived>::Scalar>>(        \
        mat.derived());                                                                                                \
  }
...
CUMAT_NAMESPACE ExtractComplexPartOp<_Child, false, false> real(const CUMAT_NAMESPACE MatrixBase<_Child>& mat)
{
  ...
  CUMAT_STATIC_ASSERT(
      CUMAT_NAMESPACE internal::NumTraits<typename CUMAT_NAMESPACE internal::traits<_Child>::Scalar>::IsComplex,
