...
struct ReductionEvaluatorHelper<_Input, _Output, Axis::Row>
{
	...
	static Index numEntries(const MatrixBase<_Input>& in)
	{
		return in.rows();
	}
	static Index numBatches(const MatrixBase<_Input>& in)
	{
		return in.cols() * in.batches();
	}
};
// column reduction
template <typename _Input, typename _Output>
...
struct ReductionEvaluatorHelper<_Input, _Output, Axis::Batch>
{
	...
	{
		bool isRowMajor = CUMAT_IS_ROW_MAJOR(internal::traits<_Output>::Flags);
		return StridedMatrixOutputIterator<_Output>(out, isRowMajor ? thrust::make_tuple(out.cols(), Index(1), 1) :
...
struct ReductionEvaluatorHelper<_Input, _Output, Axis::Row | Axis::Batch>
{
	static StridedMatrixInputIterator<_Input> iterIn(const MatrixBase<_Input>& in)
	{
		return StridedMatrixInputIterator<_Input>(in, thrust::make_tuple(1, in.rows() * in.batches(), in.rows()));
	}
	static typename _Output::Scalar* iterOut(_Output& out)
	{
		return out.data();
...
struct ReductionEvaluatorHelper<_Input, _Output, Axis::Column | Axis::Batch>
{
	static StridedMatrixInputIterator<_Input> iterIn(const MatrixBase<_Input>& in)
	{
		return StridedMatrixInputIterator<_Input>(in, thrust::make_tuple(in.cols() * in.batches(), 1, in.cols()));
	}
	static typename _Output::Scalar* iterOut(_Output& out)
	{
		return out.data();
...
struct ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Segmented>
{
	static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
	{
		...
		CUMAT_SAFE_CALL(cub::DeviceSegmentedReduce::Reduce(NULL, temp_storage_bytes, iterIn, iterOut, numBatches,
																											 iterOffsets, iterOffsets + 1, op, initial, ctx.stream(),
																											 CUMAT_CUB_DEBUG));
		DevicePointer<uint8_t> temp_storage(temp_storage_bytes);
		CUMAT_SAFE_CALL(cub::DeviceSegmentedReduce::Reduce(static_cast<void*>(temp_storage.pointer()), temp_storage_bytes,
																											 iterIn, iterOut, numBatches, iterOffsets, iterOffsets + 1, op,
																											 initial, ctx.stream(), CUMAT_CUB_DEBUG));
...
struct ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Device<MiniBatch>>
{
	static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
	{
		...
		cub::DeviceReduce::Reduce(nullptr, temp_storage_bytes, iterIn, iterOut, int(numEntries), op, initial,
															substreams[0]);
		...
		for (Index b = 0; b < numBatches; ++b)
		{
			...
			cub::DeviceReduce::Reduce(temp_storage[i].pointer(), temp_storage_bytes, iterIn + (numEntries * b), iterOut + b,
																int(numEntries), op, initial, substreams[i]);
		}
		// add sync points
		for (int b = 0; b < Bmin; ++b)
		{
...
struct ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Device<1>>
{
	static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
	{
		...
		cub::DeviceReduce::Reduce(nullptr, temp_storage_bytes, iterIn, iterOut, int(numEntries), op, initial, mainStream);
		// perform reduction
		temp_storage = DevicePointer<uint8_t>(temp_storage_bytes);
		for (Index b = 0; b < numBatches; ++b)
		{
			cub::DeviceReduce::Reduce(temp_storage.pointer(), temp_storage_bytes, iterIn + (numEntries * b), iterOut + b,
																int(numEntries), op, initial, mainStream);
		}
...
__global__ void ReduceThreadKernel(dim3 virtual_size, _Input input, _Output output, _Op op, int N)
{
...
struct ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Thread>
{
	static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
	{
		...
		Context& ctx = Context::current();
		KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
				numBatches, kernels::ReduceThreadKernel<decltype(iterIn), decltype(iterOut), _Op, _Scalar>);
		kernels::ReduceThreadKernel<decltype(iterIn), decltype(iterOut), _Op, _Scalar>
				<<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(cfg.virtual_size, iterIn, iterOut, op,
																																		 int(numEntries));
		CUMAT_CHECK_ERROR();
...
{
template <typename _Input, typename _Output, typename _Op, typename _Scalar>
__global__ void ReduceWarpKernel(dim3 virtual_size, _Input input, _Output output, _Op op, _Scalar initial, Index N)
{
...
struct ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Warp>
{
	static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
	{
		...
		Context& ctx = Context::current();
		KernelLaunchConfig cfg = ctx.createLaunchConfig1D(
				numBatches * 32, kernels::ReduceWarpKernel<decltype(iterIn), decltype(iterOut), _Op, _Scalar>);
		kernels::ReduceWarpKernel<decltype(iterIn), decltype(iterOut), _Op, _Scalar>
				<<<cfg.block_count, cfg.thread_per_block, 0, ctx.stream()>>>(cfg.virtual_size, iterIn, iterOut, op, initial,
																																		 numEntries);
		CUMAT_CHECK_ERROR();
...
__global__ void ReduceBlockKernel(dim3 virtual_size, _Input input, _Output output, _Op op, _Scalar initial, Index N)
{
...
struct ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Block<BlockSize>>
{
	static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
	{
		...
		CUMAT_LOG_DEBUG("Best potential occupancy for "
										<< typeid(kernels::ReduceBlockKernel<decltype(iterIn), decltype(iterOut), _Op, _Scalar, BlockSize>)
													 .name()
										<< " found to be: blocksize=" << bestBlockSize << ", gridSize=" << minGridSize);
		KernelLaunchConfig cfg = { dim3(internal::narrow_cast<unsigned>(numBatches), 1, 1),
	...
	static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
	{
		...
		switch (alg)
		{
			case ReductionAlgorithm::Segmented:
				ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Segmented>::eval(in, out, op, initial);
				break;
			case ReductionAlgorithm::Thread:
				ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Thread>::eval(in, out, op, initial);
				break;
			case ReductionAlgorithm::Warp:
				ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Warp>::eval(in, out, op, initial);
				break;
			case ReductionAlgorithm::Block256:
				ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Block<256>>::eval(in, out, op, initial);
				break;
			case ReductionAlgorithm::Device1:
				ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Device<1>>::eval(in, out, op, initial);
				break;
			case ReductionAlgorithm::Device2:
				ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Device<2>>::eval(in, out, op, initial);
				break;
			case ReductionAlgorithm::Device4:
				ReductionEvaluator<_Input, _Output, _Axis, _Op, _Scalar, ReductionAlg::Device<4>>::eval(in, out, op, initial);
				break;
			default:
...
struct ReductionEvaluatorSpecial<_Input, _Output, 0, _Op, _Scalar>
{
	static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
	{
		internal::Assignment<_Output, _Input, AssignmentMode::ASSIGN, typename internal::traits<_Output>::DstTag,
												 typename internal::traits<_Input>::SrcTag>::assign(out, in);
...
struct ReductionEvaluatorSpecial<_Input, _Output, Axis::Row | Axis::Column | Axis::Batch, _Op, _Scalar>
{
	...
	static void eval(const MatrixBase<_Input>& in, _Output& out, const _Op& op, const _Scalar& initial)
	{
		...
		size_t temp_storage_bytes = 0;
		CUMAT_SAFE_CALL(
				cub::DeviceReduce::Reduce(NULL, temp_storage_bytes, iterIn, iterOut, num_items, op, initial, ctx.stream()));
		DevicePointer<uint8_t> temp_storage(temp_storage_bytes);
		CUMAT_SAFE_CALL(cub::DeviceReduce::Reduce(static_cast<void*>(temp_storage.pointer()), temp_storage_bytes, iterIn,
																							iterOut, num_items, op, initial, ctx.stream(), CUMAT_CUB_DEBUG));
...
public:
	ReductionOp_DynamicSwitched(const MatrixBase<_Child>& child, int axis, const _ReductionOp& op,
															const Scalar& initialValue)
		: child_(child.derived()), axis_(axis), op_(op), initialValue_(initialValue)
	{
	}
	...
	void evalTo(MatrixBase<Derived>& m) const
	{
		...
		CUMAT_LOG_DEBUG("Evaluate reduction expression "
										<< typeid(derived()).name() << "\n rows=" << m.rows() << ", cols=" << m.cols()
										<< ", batches=" << m.batches() << ", axis=" << ((axis_ & Axis::Row) ? "R" : "")
										<< ((axis_ & Axis::Column) ? "C" : "") << ((axis_ & Axis::Batch) ? "B" : ""));

		// simplify axis, less segments are better
		const int axisSimplified = axis_ == 0 ?
																	 0 :
		...
		{
			case 0:
				internal::ReductionEvaluator<_Child, Derived, 0, _ReductionOp, Scalar, _Algorithm>::eval(child_, m.derived(),
																																																 op_, initialValue_);
				break;
			case 1:
				internal::ReductionEvaluator<_Child, Derived, 1, _ReductionOp, Scalar, _Algorithm>::eval(child_, m.derived(),
																																																 op_, initialValue_);
				break;
			case 2:
				internal::ReductionEvaluator<_Child, Derived, 2, _ReductionOp, Scalar, _Algorithm>::eval(child_, m.derived(),
																																																 op_, initialValue_);
				break;
			case 3:
				internal::ReductionEvaluator<_Child, Derived, 3, _ReductionOp, Scalar, _Algorithm>::eval(child_, m.derived(),
																																																 op_, initialValue_);
				break;
			case 4:
				internal::ReductionEvaluator<_Child, Derived, 4, _ReductionOp, Scalar, _Algorithm>::eval(child_, m.derived(),
																																																 op_, initialValue_);
				break;
			case 5:
				internal::ReductionEvaluator<_Child, Derived, 5, _ReductionOp, Scalar, _Algorithm>::eval(child_, m.derived(),
																																																 op_, initialValue_);
				break;
			case 6:
				internal::ReductionEvaluator<_Child, Derived, 6, _ReductionOp, Scalar, _Algorithm>::eval(child_, m.derived(),
																																																 op_, initialValue_);
				break;
			case 7:
				internal::ReductionEvaluator<_Child, Derived, 7, _ReductionOp, Scalar, _Algorithm>::eval(child_, m.derived(),
																																																 op_, initialValue_);
				break;
			default:
				throw std::invalid_argument(__FILE__
																		":" CUMAT_STR(__LINE__) ": Invalid argument, axis must be between 0 and 7, but "
																														"is " +
																		std::to_string(axis_));
		}
...
template <typename _Child, typename _ReductionOp, int _Axis, typename _Algorithm>
class ReductionOp_StaticSwitched
	: public MatrixBase<ReductionOp_StaticSwitched<_Child, _ReductionOp, _Axis, _Algorithm>>
{
public:
...
public:
	ReductionOp_StaticSwitched(const MatrixBase<_Child>& child, const _ReductionOp& op, const Scalar& initialValue)
		: child_(child.derived()), op_(op), initialValue_(initialValue)
	{
	...
	void evalTo(MatrixBase<Derived>& m) const
	{
		...
		// simplify axis, less segments are better
		constexpr int AxisSimplified = _Axis == 0 ?
																			 0 :
		...
		CUMAT_LOG_DEBUG("Evaluate reduction expression "
										...
										<< ", axis=" << ((AxisSimplified & Axis::Row) ? "R" : "")
										<< ((AxisSimplified & Axis::Column) ? "C" : "") << ((AxisSimplified & Axis::Batch) ? "B" : ""));

		// compile-time switch to the implementations
		internal::ReductionEvaluator<_Child, Derived, AxisSimplified, _ReductionOp, Scalar, _Algorithm>::eval(
				child_, m.derived(), op_, initialValue_);
		CUMAT_LOG_DEBUG("Evaluation done");
	}
};
