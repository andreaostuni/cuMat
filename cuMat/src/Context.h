...
// #include "../../third-party/cub/util_allocator.cuh" //No need to add cub to the global include (would clash e.g. with
// other Eigen versions)
#endif
...
#define CUMAT_KERNEL_1D_LOOP(i, virtual_size)                                                                          \
	for (CUMAT_NAMESPACE Index i = blockIdx.x * blockDim.x + threadIdx.x; i < virtual_size.x;                            \
...
#define CUMAT_KERNEL_2D_LOOP(i, j, virtual_size)                                                                       \
	for (CUMAT_NAMESPACE Index __i = blockIdx.x * blockDim.x + threadIdx.x; __i < virtual_size.x * virtual_size.y;       \
...
#define CUMAT_KERNEL_3D_LOOP(i, j, k, virtual_size)                                                                    \
	for (CUMAT_NAMESPACE Index __i = blockIdx.x * blockDim.x + threadIdx.x;                                              \
			 __i < virtual_size.x * virtual_size.y * virtual_size.z; __i += blockDim.x * gridDim.x)                          \
	{                                                                                                                    \
...
public:
	Context(int device = 0) : device_(device), stream_(nullptr)
	{
#if 0
		...
		CUMAT_LOG_DEBUG("Context initialized for thread 0x" << std::hex << std::this_thread::get_id() << ", stream: 0x"
																												<< stream_);
	}
...
#if CUMAT_CONTEXT_DEBUG_MEMORY == 1
	// For testing only
	int getAliveHostPointers() const
	{
		return allocationsHost_;
	}
	// For testing only
	int getAliveDevicePointers() const
	{
		return allocationsDevice_;
	}
#endif
	...
	KernelLaunchConfig createLaunchConfig1D(Index size, T func) const
	{
		...
		CUMAT_LOG_DEBUG("Best potential occupancy for " << typeid(T).name() << " found to be: blocksize=" << bestBlockSize
																										<< ", gridSize=" << minGridSize);
		KernelLaunchConfig cfg = { dim3(size_, 1, 1), dim3(bestBlockSize, 1, 1), dim3(minGridSize, 1, 1) };
		return cfg;
#endif
	}
	...
	KernelLaunchConfig createLaunchConfig2D(unsigned int sizex, unsigned int sizey, T func) const
	{
		...
		CUMAT_LOG_DEBUG("Best potential occupancy for " << typeid(T).name() << " found to be: blocksize=" << bestBlockSize
																										<< ", gridSize=" << minGridSize);
		KernelLaunchConfig cfg = { dim3(sizex, sizey, 1), dim3(bestBlockSize, 1, 1), dim3(minGridSize, 1, 1) };
		return cfg;
	}
	...
	KernelLaunchConfig createLaunchConfig3D(unsigned int sizex, unsigned int sizey, unsigned int sizez, T func) const
	{
		...
		CUMAT_LOG_DEBUG("Best potential occupancy for " << typeid(T).name() << " found to be: blocksize=" << bestBlockSize
																										<< ", gridSize=" << minGridSize);
		KernelLaunchConfig cfg = { dim3(sizex, sizey, sizez), dim3(bestBlockSize, 1, 1), dim3(minGridSize, 1, 1) };
		return cfg;
	}
};
...
public:
	Event() : event_(nullptr), counter_(new size_t(1))
	{
		CUMAT_SAFE_CALL(cudaEventCreate(&event_));
	}

	Event(const Event& rhs) : event_(rhs.event_), counter_(rhs.counter_)
	{
		assert(counter_);
		++(*counter_);
	}

	Event(Event&& rhs) noexcept : event_(std::move(rhs.event_)), counter_(std::move(rhs.counter_))
	{
		rhs.event_ = nullptr;
